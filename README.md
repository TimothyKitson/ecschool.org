...
public/          ← what Netlify serves (publish directory)
  assets/css     site.css — all styling (edit this one)
                 site.<hash>.css — generated by build.py; the pages link to it
...
**Why the stylesheet has a hash in its name.** `site.css` is served with a
one-year cache, so if the pages linked to it by that plain name a returning
visitor would keep using their cached copy against new HTML — which breaks the
layout, not just the colours. `build.py` therefore copies it to
`site.<hash>.css` and links *that*, so any change to the CSS produces a new
URL nobody has cached. Always run `python3 build.py` after editing
`site.css`, or the pages will still point at the previous hash.
